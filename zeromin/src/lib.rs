...
// This is what we are going to focus on, a simple async hello world from Rust -> RAN Online

#[no_mangle]
pub async fn async_operation_callback(cha_name: String ,seconds: u64) -> String {
    tokio::time::sleep(tokio::time::Duration::from_secs(seconds)).await;
    format!("Hello {} from rust!, I've waited {} seconds", cha_name, seconds)
}

#[no_mangle]
pub extern "C" fn perform_async_operation(char_name: *const c_char, seconds: u64, callback: extern "C" fn(*const c_char, usize)) {
    ...
    std::thread::spawn(move || {
        ...
        let output: String = rt.block_on(async {
            async_operation_callback(cha_name_str, seconds).await
        });
